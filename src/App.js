    ...
    <Shell>
      ...
      <div className="w-full flex grow items-center justify-around">
        <div className="flex flex-col items-center gap-4 px-4 py-8 bg-gray-100 shadow-inner">
          <Video
            onClick={() => handleVideoClick(localStream)}
            source={localStream}
          ...
          />
          <button
            className="aspect-square w-fit flex items-center justify-center p-4 rounded-full bg-red-500 shadow-md hover:scale-110 active:scale-100 transition-transform"
          ...
          <div className="flex gap-x-6">
            ...
            {!calling && (
              <input
                ...
                placeholder="Call #. Empty For New Call..."
                type="text"
                className="shadow-inner rounded-full px-4 py-1 w-full outline-none border-none text-lg text-center"
              ></input>
            )}
          </div>
        </div>
...
function CallButton({ calling, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`aspect-square w-fit flex items-center justify-center p-4 rounded-full ${
        calling ? "bg-white" : "bg-green-500"
      } shadow-md hover:scale-110 active:scale-100 transition-transform`}
    >
      {calling ? (
        <RiPhoneFill
          color="rgb(239 68 68)"
